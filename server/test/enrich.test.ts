...
test('vet: the model says whether a listing is an event, and why not', () => {
  ...
  assert.deepEqual(schema.required, ['listingIs', 'isEvent']);
  assert.equal(readVerdict({ listingIs: 'a car show', isEvent: true }, ['vet']).notEvent, '');
  assert.equal(readVerdict({ listingIs: 'a race report', isEvent: false }, ['vet']).notEvent, 'a race report');
  assert.equal(readVerdict({ listingIs: '', isEvent: false }, ['vet']).notEvent, 'not an event', 'a no without a reason is still a no');
